...
// Array dos inputs (Com as caracteristicad individuais de validação deles)
let inputs = {
  name: {
    maxLength: 40,
    required: true,
...
// Função para validar um input default (de texto generico)
function defaultValidation(input, name){
  let trimmed = input.value.trim(); //Saber quantos caracteres tem (tira os espaços)
  let validation = inputs[name]; //inputs[name] retorna o objeto do array inputs que vai ser validado.

  if(validation.required && trimmed.length === 0){ //Se validation é required && Se mesmo sendo required, o campo está vazio.
    return false;
  }

  if(validation.maxLength && trimmed.length > validation.maxLength){ // Se validation tem um tamanho limite maximo... && Se esse limite
    //maximo foi ultrapassado.
    return false;
  }
...
function validateInput(inputName){
  let inputType = inputs[inputName].type; // Vai ser usado posteriormente no validationStrategies
  let input = document.querySelector(`[name=${inputName}]`) // Abstração para chegar no DOM do input usando o name

  if(inputType){
    let validationFunction = validationStrategies[inputType]; // Acessa o array e retorna seu valor..: ex:
    // validationStrategies[date] retorna dateValidation
    return validationFunction(input, inputName); // Chama a função certa e faz a devida validação
  }

  return validationStrategies.default(input, inputName); // Se não existir um input type... o codigo chega aqui e chama a função
  // defaultValidation;
}
